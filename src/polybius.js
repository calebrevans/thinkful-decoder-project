  ...
  function polybius(input, encode = true) {
    ...
    if (encode) {
      return input.reduce((result, char) => {
        ...
        if (code >= 97 && code <= 122) {
          return `${result}${encoderMap[char]}`;
        }
        return `${result}${char}`;
      }, "");
    }

    const spaceCount = input.reduce((acc, char) => {
      return char !== " " ? acc + 1 : acc;
    }, 0);

    if (spaceCount % 2 !== 0) return false;
    ...
    for (let i = 0; i < input.length; i += 2) {
      if (input[i] !== " ") {
        ...
        const decoded = decoderMap[encoded];
        decodedStr = `${decodedStr}${decoded}`;
      } else {
        i += 1;
        const encoded = input[i] + input[i + 1];
        const decoded = decoderMap[encoded];
